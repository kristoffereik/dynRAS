...
def chemODE_BIO(Tank,params,t):
    ...
    day = t / 60 / 60 / 24
    #K1=(johnson 1982)
    # simulation of the experimental set up from Jafari et al. 2024
    if day <= 14:
        T_HCO3 = 200/50.04
...
def chemODE_BIO_NaOH(Tank, params, t):
    #simulation case 2
    # NaOH only
    #The simulation for dosing NaOH only is a bit slower toward the end when targetting an alkalinity of 200
    Growth=growth_bact(Tank,params)
    g_AOB=Growth[0]
    g_NOB=Growth[1]
    d_AOB=Growth[2]
...
def chemODE_BIO_pH_control(Tank, params, t):
    #set the pH with either NaOH or HCO3 (supplementary figure)
    Growth=growth_bact(Tank,params)
    g_AOB=Growth[0]
    g_NOB=Growth[1]
    d_AOB=Growth[2]
