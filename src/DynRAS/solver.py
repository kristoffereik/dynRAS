...
import numpy as np
import pandas as pd
from .ChemODE_BIO import \
    chemODE_BIO  # simulation case 1 modify uncomment function for the right hand side of for the ODE of the biofilter class to run it
from .ChemODE_BIO import \
    chemODE_BIO_HCO3  # simulation case 1 HCO3 dosing only uncomment the function for the right hand side of for the ODE of the biofilter class to run it
from .ChemODE_BIO import \
    chemODE_BIO_NaOH  # simulation case 1 NaOH dosing only uncomment the function for the right hand side of for the ODE of the biofilter class to run it
from .ChemODE_BIO import chemODE_BIO_alk_control
from .ChemODE_BIO import \
    chemODE_BIO_pH_control  # set a pH threshold and dose NaOH or HCO3 to maintain the pH at the setpoint uncomment the function for the right hand side of for the ODE of the biofilter class to run it
...
class Solver:
    ...
    def __init__(self, params):
        ...
        self.FishTank = fish_tank(self.CO2aq_FT, self.HCO3_FT, self.CO32_FT, self.H_FT, self.OH_FT, self.NH4_FT,
                                  self.NH3_FT, self.NO2_FT, [], 1000, self.Fishweight_FT, self.Fish_number_FT,
                                  self.Fish_Biomass_FT)
        ...
        self.DGS = degasser(self.CO2aq_DGS, self.HCO3_DGS, self.CO32_DGS, self.H_DGS, self.OH_DGS, self.NH4_DGS,
                            self.NH3_DGS, self.NO2_DGS, self.B1, 700, [])

        self.exchange_rate = (self.B1.V + self.DGS.V + self.FishTank.V) * 0.25 / (
                    60 * 60 * 24)  # 25% of the total volume is exchanged every day this can be change to any % of the total volume

        self.Sump_1 = Sump(self.CO2aq_Sump, self.HCO3_Sump, self.CO32_Sump, self.H_Sump, self.OH_Sump, self.NH4_Sump,
                           self.NH3_Sump, self.NO2_Sump, self.V_Sump, self.exchange_rate)
        self.DGS = degasser(self.CO2aq_DGS, self.HCO3_DGS, self.CO32_DGS, self.H_DGS, self.OH_DGS, self.NH4_DGS,
                            self.NH3_DGS, self.NO2_DGS, self.B1, 700, self.Sump_1)
        ...
        self.S0 = [self.CO2aq_FT, self.HCO3_FT, self.CO32_FT, self.H_FT, self.OH_FT, self.NH4_FT, self.NH3_FT,
                   self.NO2_FT, self.Fishweight_FT, self.Fish_Biomass_FT, self.CO2aq_B1, self.HCO3_B1, self.CO32_B1,
                   self.H_B1, self.OH_B1, self.NH4_B1, self.NH3_B1, self.NO2_B1, self.AOB_B1, self.NOB_B1,
                   self.CO2aq_DGS, self.HCO3_DGS, self.CO32_DGS, self.H_DGS, self.OH_DGS, self.NH4_DGS, self.NH3_DGS,
                   self.NO2_DGS]

    def solve(self):
        for cycle in range(0,
                           10):  # solve the system for 10 cycles of 14 days, after each cycle the fish biomass is reduced back to 50kg/m3, the simulation is in seconds

            tspan = [14 * 24 * 60 * 60 * cycle, 14 * 24 * 60 * 60 * (cycle + 1)]
            MAlocal = solve_ivp(fun=chem, t_span=tspan, y0=self.S0,
                                args=(self.params, self.FishTank, self.B1, self.DGS),
                                method="BDF")  # if the solver does not converge try using the method "LSODA" with rtol=1e-12;atol=1e-12
        ...
        self.df_results = pd.DataFrame(MA.y.T,
                                       columns=['CO2aq_FT', 'HCO3_FT', 'CO32_FT', 'H_FT', 'OH_FT', 'NH4_FT', 'NH3_FT',
        ...
        self.df_results.to_csv('results.csv',
                               index=False)  # alkalinity is calculated as follow Alkalinity = ([HCO3-] + 2*[CO3--] + [OH-]) * 50.04 to have it in mg/L
